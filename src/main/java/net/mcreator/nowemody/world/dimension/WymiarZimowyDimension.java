...
package net.mcreator.nowemody.world.dimension;
...
import net.mcreator.nowemody.procedures.WymiarZimowyPlayerEntersDimensionProcedure;
import net.mcreator.nowemody.item.WymiarZimowyItem;
...
public class WymiarZimowyDimension extends NowemodyModElements.ModElement {
	@ObjectHolder("nowemody:wymiar_zimowy_portal")
	public static final CustomPortalBlock portal = null;
	public WymiarZimowyDimension(NowemodyModElements instance) {
		super(instance, 4);
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("nowemody:wymiar_zimowy"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("wymiar_zimowy_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("wymiar_zimowy_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())),
					0, 1).setRegistryName("wymiar_zimowy_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new WymiarZimowyItem().setRegistryName("wymiar_zimowy"));
	}
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("wymiar_zimowy_portal");
		}
		...
		public void animateTick(BlockState state, World world, BlockPos pos, Random random) {
			...
			if (random.nextInt(110) == 0)
				world.playSound(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5,
						(net.minecraft.util.SoundEvent) ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation(("block.snow.fall"))),
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("nowemody:wymiar_zimowy"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos, RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("nowemody:wymiar_zimowy")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == Blocks.ICE.getDefaultState().getBlock();
		};
	...
	public void onPlayerChangedDimensionEvent(PlayerEvent.PlayerChangedDimensionEvent event) {
		...
		if (event.getTo() == RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("nowemody:wymiar_zimowy"))) {
			{
				...
				WymiarZimowyPlayerEntersDimensionProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
